"""
This file contains code adapted from Salesforce LAVIS.
...
Based on Hugging Face Transformers BERT code:
https://github.com/huggingface/transformers/blob/v4.15.0/src/transformers/models/bert

Modifications:
Copyright (c) 2026, Chang Shen.
Modifications for GopherEye model assembly and compatibility.

For full BSD-3-Clause license text, see LICENSE or:
...
def apply_chunking_to_forward(
    ...
    if len(input_tensors) == 0:
        raise ValueError("input_tensors has to be a non-empty tuple of tensors")
    ...
    if chunk_size > 0:
        ...
        for input_tensor in input_tensors:
            if input_tensor.shape[chunk_dim] != tensor_shape:
                raise ValueError(
                    "All input tensors have to be the same shape on the chunked dimension"
                )

        if tensor_shape % chunk_size != 0:
            raise ValueError(
                f"The dimension to be chunked {tensor_shape} has to be a multiple of the chunk size {chunk_size}"
            )
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def invert_attention_mask(self, encoder_attention_mask: Tensor) -> Tensor:
        if encoder_attention_mask.dim() == 3:
            ...
            raise ValueError(
                "Wrong shape for encoder_attention_mask "
                f"(shape {encoder_attention_mask.shape})"
            )
