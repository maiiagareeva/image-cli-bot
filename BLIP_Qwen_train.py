...
def main():
    ...
    model = build_model(model_cfg=model_cfg, device=device,train_cfg=train_cfg)
    ...
    image_processor.save_pretrained(train_cfg.new_model_dir)
    ...
    if model.query_mixer is not None:
        torch.save(
            model.query_mixer.state_dict(),
            os.path.join(train_cfg.new_model_dir, "query_mixer.pt")
        )
    save_meta = {
        "base_model": model_cfg.base_model,
        "blip2_model": model_cfg.blip2_model,
        "qformer_dim": model.blip.qformer_dim,
        "prefix_len": model.blip.prefix_len,
        "use_query_mixer": model.query_mixer is not None,
        "use_weighted_loss": model.use_weighted_loss,
    }
    import json
    with open(os.path.join(train_cfg.new_model_dir, "model_meta.json"), "w", encoding="utf-8") as f:
    ...
    with open(os.path.join(train_cfg.new_model_dir, "train_config_snapshot.yaml"), "w", encoding="utf-8") as f:
        f.write(yaml_text)
